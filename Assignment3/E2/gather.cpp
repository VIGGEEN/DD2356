...
int main(int argc, char **argv) {
    ...
    std::vector<int> recvbuffer(world_size);
    MPI_Gather(&count, 1, MPI_INT, &recvbuffer.front(), 1, MPI_INT, 0, MPI_COMM_WORLD);
    if (world_rank == 0) {
        count = std::accumulate(recvbuffer.begin(), recvbuffer.end(), 0);
